...
const TEAM_NAME = "Alevín masculino";
...
const DEFAULT_VISIBLE_TEAMS = [
  ...
  { id: "infantil-masculino", name: "Infantil masculino" },
  { id: "cadete-masculino", name: "Cadete masculino" },
  { id: "infantil-femenino", name: "Infantil femenino" },
  { id: "cadete-femenino", name: "Cadete femenino" }
];
...
const LEGACY_TEAM_ID_MAP = {
  "athletic-club": "alevin-masculino",
  "ce-ferreries-cadete-a": "cadete-masculino",
  "ce-ferreries-cadete-preferente": "cadete-masculino",
  "ce-ferreries-infantil-preferente": "infantil-masculino",
  "ce-ferreries-alevin-a": "alevin-masculino",
  "ce-ferreries-alevin-b": "alevin-masculino",
  "ce-ferreries-benjamin-a": "alevin-masculino",
  "ce-ferreries-benjamin-b": "alevin-masculino",
  "ce-ferreries-prebenjamin": "alevin-masculino"
};
const REMOVED_TEAM_IDS = new Set([
  ...Object.keys(LEGACY_TEAM_ID_MAP),
...
function migrateTeamId(teamId) {
  const id = String(teamId || DEFAULT_TEAM_ID);
  return LEGACY_TEAM_ID_MAP[id] || id;
}

function isRemovedTeam(item) {
  return (
    REMOVED_TEAM_IDS.has(String(item?.id || "")) ||
    REMOVED_TEAM_NAMES.has(String(item?.name || "").trim().toLowerCase())
  );
}

function mergeDefaultVisibleTeams(teams, currentVersion = 0) {
  if ((Number(currentVersion) || 0) < DEFAULT_TEAMS_VERSION) {
    return structuredClone(DEFAULT_VISIBLE_TEAMS);
  }
  const visibleTeams = (teams?.length ? teams : structuredClone(DEFAULT_VISIBLE_TEAMS)).filter(
    (item) => !isRemovedTeam(item)
  );
  return visibleTeams;
}
...
function loadState() {
  try {
    ...
    return {
      ...
      ...saved,
      lastModifiedAt:
        shouldResetPlayers || shouldResetMatches ? Date.now() : Number(saved.lastModifiedAt) || 1,
      defaultTeamsVersion: DEFAULT_TEAMS_VERSION,
      playersResetVersion: PLAYERS_RESET_VERSION,
      matchesResetVersion: MATCHES_RESET_VERSION,
      activeView,
      teams,
      activeTeamId,
      selectedPlayerId: shouldResetPlayers ? "" : saved.selectedPlayerId || "",
      selectedMatchId: shouldResetMatches ? "" : saved.selectedMatchId || "",
      players: shouldResetPlayers
        ? []
        : saved.players?.map((item) => ({
            ...item,
...
async function pullRemoteState() {
  ...
  try {
    ...
    const shouldIgnoreRemoteMatches =
      (Number(remoteAppState?.matchesResetVersion) || 0) < MATCHES_RESET_VERSION;
...
function fromRemotePlayer(item, localItem = {}) {
  ...
  return {
    id: item.id,
    teamId: migrateTeamId(remoteProfile.teamId || localItem.teamId),
...
function applyRemoteAppState(remoteAppState, preferLocal) {
  if (Array.isArray(remoteAppState.teams) && remoteAppState.teams.length) {
    const remoteTeams = remoteAppState.teams
      ...
      .filter((item) => !isRemovedTeam(item))
  ...
  state.players = state.players.map((item) => {
    ...
    return {
      ...item,
      teamId: migrateTeamId(primary.teamId || secondary.teamId),
...
function fromRemoteMatch(item, lineupData, plan) {
  return {
    ...
    teamId: migrateTeamId(lineupData?.teamId),
...
function syncTeamsFromData() {
  ...
  [...state.players, ...state.matches].forEach((item) => {
    ...
    if (isRemovedTeam({ id, name: item.teamName })) return;
    if (!known.has(id)) {
...
function renderStandings() {
  ...
  views.standings.innerHTML = `
    <div class="section-intro">
      <div>
        <h2>Clasificación</h2>
        <p class="meta">No hay ninguna clasificación registrada.</p>
      </div>
    </div>
    ${
      table.length
